...
int main(int argc, char *argv[]) {
    ...
    for (int run = 0; run < NUM_RUNS; run++) {
        ...
        if (strcmp(argv[1], "smooth") == 0) apply_smoothing(input, output);
        else if (strcmp(argv[1], "sharpen") == 0) apply_sharpening(input, output);
        else if (strcmp(argv[1], "edge") == 0) apply_edge_detection(input, output);
        else if (strcmp(argv[1], "emboss") == 0) apply_emboss(input, output);
        else {
    ...
    printf("\nAverage Execution Time (OpenMP): %.6f seconds\n", avg_time);
